...
static s32 initrd_read(Vfs_Node* vfs_node, u32 offset, u32 size, void* buf) {
	...
	util_memcpy(buf, file_data + offset, size_to_read);
}
...
static s32 initrd_readdir(Vfs_Node* vfs_node, u32 index, Vfs_Dirent* dirent) {
	...
	util_strcpy(dirent->name, initrd_files_nodes[index].name);
...
Vfs_Node* initrd_init() {
	...
	for (u32 i = 0; i < num_files; ++i) {
		...
		util_strcpy(initrd_files_nodes[i].name, headers[i].file_name);
	...
	util_strcpy(initrd_root_node->name, "initrd");
